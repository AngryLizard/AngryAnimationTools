...
TWeakPtr<ISequencer> FModifierMirror::GetSequencer() const
{
	TWeakPtr<ISequencer> WeakSequencer = nullptr;

	FAngryAnimationToolsEditorModule& AngryAnimationToolsEditor = FModuleManager::LoadModuleChecked<FAngryAnimationToolsEditorModule>("AngryAnimationToolsEditor");
	TWeakPtr<ISequencer> CurrentSequencer = AngryAnimationToolsEditor.GetSequencer();
...
void FModifierMirror::FlipLeftToRight(bool bWithOffet)
{
	TWeakPtr<ISequencer> Sequencer = GetSequencer();
	if (!Sequencer.IsValid() || !Sequencer.Pin()->GetFocusedMovieSceneSequence())
	{
...
void FModifierMirror::FlipRightToLeft(bool bWithOffet)
{
	TWeakPtr<ISequencer> Sequencer = GetSequencer();
	if (!Sequencer.IsValid() || !Sequencer.Pin()->GetFocusedMovieSceneSequence())
	{
